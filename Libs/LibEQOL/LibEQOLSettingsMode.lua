...
function lib:CreateScrollDropdown(cat, data)
	...
	local initializer = Settings.CreateElementInitializer("LibEQOL9d98999_ScrollDropdownTemplate", {
...
function lib:CreateSoundDropdown(cat, data)
	...
	local setting = registerSetting(
		...
		local initializer = Settings.CreateElementInitializer("LibEQOL9d98999_SoundDropdownTemplate", {
			setting = setting,
...
function lib:CreateColorOverrides(cat, data)
	...
	local initializer = Settings.CreateElementInitializer("LibEQOL9d98999_ColorOverridesPanelNoHead", {
...
function lib:CreateMultiDropdown(cat, data)
	...
	local initializer = Settings.CreateElementInitializer("LibEQOL9d98999_MultiDropdownTemplate", {
		label = data.name or data.text or data.key,
		options = data.values,
		optionfunc = data.optionfunc,
		order = data.order,
...
function lib:CreateText(cat, text, extra)
	...
	local init = Settings.CreateElementInitializer(
		"LibEQOL9d98999_SettingsListSectionHintTemplate",
		{ name = name }
	)
	addSearchTags(init, data.searchtags or name, name)
	applyParentInitializer(init, data.parent, data.parentCheck)
