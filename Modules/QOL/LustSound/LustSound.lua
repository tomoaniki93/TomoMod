...
local function OnLustDetected(source)
    ...
    if db.showChat then
        print("|cff2e9dd8TomoMod|r |cffff4400\226\153\170 Bloodlust d\195\169tect\195\169 !|r")
    end

    if GetTime() < suppressUntil then
        if db.debug then
            print("|cff2e9dd8LustSound|r Trigger suppressed (login settle)")
        end
    else
        if db.debug then
            print("|cff2e9dd8LustSound|r Trigger: " .. (source or "unknown"))
        end
        DoPlaySound()
    end
end
...
local function OnLustEnded()
    ...
    if db and db.showChat then
        print("|cff2e9dd8TomoMod|r |cff888888Bloodlust termin\195\169.|r")
    end
end
