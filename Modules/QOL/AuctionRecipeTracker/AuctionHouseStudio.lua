...
local function BuildSearchView(parent)
    ...
    local detail=Card(p); detail:SetPoint("TOPRIGHT",-18,-110); detail:SetPoint("BOTTOMRIGHT",-18,18); p.detail=detail
    ...
    -- Compact purchase card. The old V3.2 controls used a mix of absolute
    -- top/bottom anchors on the detail frame itself; depending on UI scale,
    -- the quantity field could still climb into the price cards. Give the
    -- purchase controls their own elastic area between prices and the button.
    ...
    local purchaseCard=Card(detail); purchaseCard:SetPoint("TOPLEFT",currentCard,"BOTTOMLEFT",0,-8); purchaseCard:SetPoint("TOPRIGHT",scanCard,"BOTTOMRIGHT",0,-8); purchaseCard:SetPoint("BOTTOMLEFT",detail.buy,"TOPLEFT",0,8); purchaseCard:SetPoint("BOTTOMRIGHT",detail.buy,"TOPRIGHT",0,8); detail.purchaseCard=purchaseCard

    local qtyLabel=Text(purchaseCard,T("buy_quantity"),8,true); qtyLabel:SetPoint("TOPLEFT",9,-9); qtyLabel:SetWidth(58); qtyLabel:SetWordWrap(false); if qtyLabel.SetMaxLines then qtyLabel:SetMaxLines(1) end; qtyLabel:SetTextColor(unpack(C.dim))
    detail.qtyBox=CreateFrame("EditBox",nil,purchaseCard,"BackdropTemplate"); detail.qtyBox:SetSize(66,22); detail.qtyBox:SetPoint("TOPLEFT",66,-4); detail.qtyBox:SetAutoFocus(false); detail.qtyBox:SetNumeric(true); detail.qtyBox:SetMaxLetters(7); detail.qtyBox:SetFont(FONT_B,9,""); detail.qtyBox:SetJustifyH("CENTER"); detail.qtyBox:SetTextInsets(4,4,0,0); Backdrop(detail.qtyBox,{0.035,0.042,0.052,1},C.border)
    ...
    detail.max=Button(purchaseCard,T("buy_max"),46,22,SetMaxPurchaseQuantity); detail.max:SetPoint("LEFT",detail.qtyBox,"RIGHT",5,0); detail.max:Hide()

    detail.type=Text(purchaseCard,"",8,false); detail.type:SetPoint("TOPRIGHT",-9,-9); detail.type:SetWidth(92); detail.type:SetJustifyH("RIGHT"); detail.type:SetWordWrap(false); if detail.type.SetMaxLines then detail.type:SetMaxLines(1) end; detail.type:SetTextColor(unpack(C.dim))

    local totalLabel=Text(purchaseCard,T("buy_total"),8,true); totalLabel:SetPoint("LEFT",9,-4); totalLabel:SetWidth(88); totalLabel:SetWordWrap(false); if totalLabel.SetMaxLines then totalLabel:SetMaxLines(1) end; totalLabel:SetTextColor(unpack(C.dim))
    detail.totalValue=Text(purchaseCard,"—",11,true); detail.totalValue:SetPoint("RIGHT",-9,-4); detail.totalValue:SetWidth(140); detail.totalValue:SetJustifyH("RIGHT"); detail.totalValue:SetWordWrap(false); if detail.totalValue.SetMaxLines then detail.totalValue:SetMaxLines(1) end

    detail.buyStatus=Text(purchaseCard,T("buy_hint"),8,false); detail.buyStatus:SetPoint("BOTTOMLEFT",9,7); detail.buyStatus:SetPoint("BOTTOMRIGHT",-9,7); detail.buyStatus:SetHeight(12); detail.buyStatus:SetJustifyH("LEFT"); detail.buyStatus:SetJustifyV("MIDDLE"); detail.buyStatus:SetWordWrap(false); if detail.buyStatus.SetMaxLines then detail.buyStatus:SetMaxLines(1) end; detail.buyStatus:SetTextColor(unpack(C.dim))

    local function LayoutColumns()
        local w=p:GetWidth() or 0
        if w<=0 then return end
        detail:SetWidth(math.max(350,math.min(410,math.floor(w*0.37))))
    end
