-- =====================================================================
-- Ported from Tui: TUI_ActionBars/actionbars/actionbars_petstance.lua
--
...
function ActionBarsOwned.UpdatePetButton(btn)
    ...
    if showOrangeBorder then
        if not btn.TUI_ActiveBorder then
            btn.TUI_ActiveBorder = btn:CreateTexture(nil, "OVERLAY", nil, 3)
            btn.TUI_ActiveBorder:SetTexture(TEXTURES.normal)
            btn.TUI_ActiveBorder:SetAllPoints(btn)
        end
        btn.TUI_ActiveBorder:SetVertexColor(1.0, 0.6, 0.0, 1.0)
        btn.TUI_ActiveBorder:Show()
    elseif btn.TUI_ActiveBorder then
        btn.TUI_ActiveBorder:Hide()
    end
...
UpdatePetBarVisibility = function()
    ...
    if barDB and barDB.enabled == false then
        ...
        if _G.TUI_UpdateFramesAnchoredTo then _G.TUI_UpdateFramesAnchoredTo("petBar") end
        return
    end
    ...
    if _G.TUI_RefreshActionBarsVisibility then
        _G.TUI_RefreshActionBarsVisibility()
    end
    local isShown = container:IsShown()
    if wasShown ~= isShown and _G.TUI_UpdateFramesAnchoredTo then
...
UpdateStanceBarLayout = function()
    ...
    if barDB and barDB.enabled == false then
        ...
        container:Hide()
        if _G.TUI_UpdateFramesAnchoredTo then _G.TUI_UpdateFramesAnchoredTo("stanceBar") end
        return
    end
    ...
    if numForms == 0 then
        ...
        container:Hide()
        if wasShown and _G.TUI_UpdateFramesAnchoredTo then
            _G.TUI_UpdateFramesAnchoredTo("stanceBar")
        end
        return
    end
    ...
    if not wasShown and _G.TUI_UpdateFramesAnchoredTo then
        _G.TUI_UpdateFramesAnchoredTo("stanceBar")
    end
end
