-- =====================================================================
-- Ported from Tui: TUI_ActionBars/actionbars/actionbars_skinning.lua
--
...
local function ShouldSuppressNativeProc()
    local db = GetDB()
    local source = db and db.global and db.global.glowSource or "TUI"
...
local function GetButtonRegions(button)
    local r = button._tomomodRegions
    if not r then
        r = {}
        button._tomomodRegions = r
    end
    local state = GetFrameState(button)
...
SkinButton = function(button, settings)
    ...
    if button.UpdateButtonArt and not button._tomomodArtHooked then
    ...
    elseif button._tomomodBridged and Bridge then
        Bridge.RemoveButton("actionbars", button)
        button._tomomodBridged = nil
    end
end
