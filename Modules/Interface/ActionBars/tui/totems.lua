-- =====================================================================
-- Ported from Tui: TUI_ActionBars/actionbars/totems.lua
--
...
local TotemBar = {}
ns.TUI_TotemBar = TotemBar

local TUICore = ns.Addon
...
local container = CreateFrame("Frame", "TUI_TotemBar", UIParent)
...
for i = 1, MAX_SLOTS do
    local btn = CreateFrame("Button", "TUI_TotemBarButton" .. i, container, "SecureActionButtonTemplate")
    btn:SetSize(36, 36)
    ...
    btn.cooldown = CreateFrame("Cooldown", "TUI_TotemBarCD" .. i, btn, "CooldownFrameTemplate")
...
local function StyleButton(btn)
    ...
    if bs > 0 then
        local bpx = (TUICore and TUICore.Pixels) and TUICore:Pixels(bs, btn) or bs
        btn.border:Show()
...
local function LayoutButtons()
    ...
    local anchoring = ns.TUI_Anchoring
    if anchoring and anchoring.ApplyFrameAnchor and TUICore
       and TUICore.db and TUICore.db.profile and TUICore.db.profile.frameAnchoring then
        local settings = TUICore.db.profile.frameAnchoring.totemBar
        if settings then
...
local function PositionContainer()
    ...
    if _G.TUI_HasFrameAnchor and _G.TUI_HasFrameAnchor("totemBar") then return end
...
container:SetScript("OnDragStop", function(self)
    ...
    if anchorX and anchorY and screenX and screenY then
        if TUICore and TUICore.PixelRound then
            db.offsetX = TUICore:PixelRound(anchorX - screenX)
            db.offsetY = TUICore:PixelRound(anchorY - screenY)
        else
...
_G.TUI_RefreshTotemBar = function()
    TotemBar:Refresh()
...
_G.TUI_ShowTotemBarPreview = function()
    TotemBar:ShowPreview()
end

_G.TUI_HideTotemBarPreview = function()
    TotemBar:HidePreview()
end

if ns.Registry then
    ns.Registry:Register("totemBar", {
        refresh = _G.TUI_RefreshTotemBar,
        priority = 20,
        group = "frames",
