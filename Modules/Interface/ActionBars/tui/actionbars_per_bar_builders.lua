-- =====================================================================
-- Ported from Tui: TUI_ActionBars/actionbars/actionbars_per_bar_builders.lua
--
...
do
    local ActionBarsPerBarBuilders = ns.TUI_ActionBarsPerBarBuilders or {}
    ns.TUI_ActionBarsPerBarBuilders = ActionBarsPerBarBuilders
    ...
    local function InitializePerBarBuilders()
        ...
        local U = ns.TUI_LayoutMode_Utils
        local P = U.PlaceRow
        ...
        local function BuildRaidMarkersBarSettings(content)
            ...
            local function RefreshRaidMarkersBar()
                if type(_G.TUI_RefreshRaidMarkersBar) == "function" then
                    _G.TUI_RefreshRaidMarkersBar()
                end
            end
        ...
        local function BuildSpecialButtonSettings(content, barKey, barDB)
            ...
            local function ShowSpecialButtonReloadPrompt()
                ...
                if gui and gui.ShowConfirmation then
                    gui:ShowConfirmation({
                        ...
                        onAccept = function()
                            if TUI and TUI.SafeReload then
                                TUI:SafeReload()
                            end
                        end,
                    })
            ...
            local function RefreshSpecialButton()
                if type(_G.TUI_RefreshExtraButtons) == "function" then
                    _G.TUI_RefreshExtraButtons()
                end
                if type(_G.TUI_RefreshActionBarFade) == "function" then
                    _G.TUI_RefreshActionBarFade()
                end
                if type(_G.TUI_UpdateFramesAnchoredTo) == "function" then
                    _G.TUI_UpdateFramesAnchoredTo(barKey)
                end
            end
            ...
            CreateCollapsible(content, ns.L["Button"], 3 * FORM_ROW + 8, function(body)
                ...
                sy = P(GUI:CreateFormToggle(body, ns.L["Enabled"], "enabled", barDB, RefreshSpecialButtonEnabled,
                    { description = ns.L["Let TUI manage this button's holder, position, scale, artwork, and mouseover behavior."] }), body, sy)
        ...
        local function BuildBarSettings(content, barKey, width)
            ...
            local function ApplyBarEnabledState(val)
                ...
                if type(_G.TUI_UpdateFramesAnchoredTo) == "function" then
                    local anchorKey = (dbKey == "microbar" and "microMenu")
                        ...
                        or dbKey
                    _G.TUI_UpdateFramesAnchoredTo(anchorKey)
                end
                if ns.TUI_ActionBarsOptions and ns.TUI_ActionBarsOptions.RefreshPreview then
                    ns.TUI_ActionBarsOptions.RefreshPreview()
                end
                ...
                CreateCollapsible(content, ns.L["Layout"], numRows * FORM_ROW + 8, function(body)
                    ...
                    if barKey == "bar1" then
                        sy = P(GUI:CreateFormCheckbox(body,
                            ...
                            function(val)
                                if _G.TUI_ApplyPageArrowVisibility then
                                    _G.TUI_ApplyPageArrowVisibility(val)
                                end
            ...
            CreateCollapsible(content, ns.L["Visual"], 7 * FORM_ROW + 8, function(body)
                ...
                P(GUI:CreateFormDropdown(body, ns.L["Pressed Effect"],
                    ...
                    { description = ns.L["Visual response when a button is pressed. Blizzard Default replays the stock animation; TUI swaps in a subtle overlay; Off disables both."] }), body, sy)
            end, sections, relayout)
