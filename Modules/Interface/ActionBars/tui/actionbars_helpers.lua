-- =====================================================================
-- Ported from Tui: TUI_ActionBars/actionbars/actionbars_helpers.lua
--
...
local function ResolveBarKeyFromButtonName(name)
    ...
    if name:match("^TUI_Bar1Button%d+$") then return "bar1" end
    if name:match("^TUI_Bar2Button%d+$") then return "bar2" end
    if name:match("^TUI_Bar3Button%d+$") then return "bar3" end
    if name:match("^TUI_Bar4Button%d+$") then return "bar4" end
    if name:match("^TUI_Bar5Button%d+$") then return "bar5" end
    if name:match("^TUI_Bar6Button%d+$") then return "bar6" end
    if name:match("^TUI_Bar7Button%d+$") then return "bar7" end
    if name:match("^TUI_Bar8Button%d+$") then return "bar8" end
    ...
    if name:match("^TUI_PetButton%d+$") or name:match("^PetActionButton%d+$") then return "pet" end
    if name:match("^TUI_StanceButton%d+$") or name:match("^StanceButton%d+$") then return "stance" end
...
function GetBarKeyFromButton(button)
    if button and button._tomomodBarKey then return button._tomomodBarKey end
...
function GetButtonIndex(button)
    if not button then return nil end
    local index = button._tomomodButtonIndex
