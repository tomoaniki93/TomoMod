...
local function CreateCastBar()
    ...
    castBar.iconBorder:SetBackdrop({ edgeFile = "Interface\\Buttons\\WHITE8x8", edgeSize = 1 })
    ...
    castBar.spellName:SetPoint("LEFT", 4, 0)
    ...
    castBar.timer:SetPoint("RIGHT", -4, 0)
...
local function UpdateAppearance()
    ...
    if db.showSpellName then castBar.spellName:Show() else castBar.spellName:Hide() end
    if db.showTargetName then castBar.targetName:Show() else castBar.targetName:Hide() end
end
...
local function UpdateFlash()
    ...
    if db.flashOnTargeted and IsPlayerTargetedByTarget() and not TomoMod_PreviewMode.IsActive() then
        if not castBar.flashAnim:IsPlaying() then castBar.flashAnim:Play() end
    else
...
local function StopCast(unit, interrupted)
    ...
    if interrupted then
        ...
        fadeOutTimer = C_Timer.NewTimer(0.5, function()
            if castBar and not TomoMod_PreviewMode.IsActive() then castBar:Hide() end
            fadeOutTimer = nil
        end)
    else
        castBar:Hide()
...
local function OnUpdate(self, elapsed)
    ...
    if TomoMod_PreviewMode.IsActive() then
        ...
        local barWidth = castBar.barWidth or 200
        if barWidth > 0 then
    ...
    if not castBar.casting or not castBar.startTime or not castBar.endTime or not castBar.duration then
        return
    end
    ...
    if castBar.channeling then
        ...
        local elapsedTime = currentTime - castBar.startTime
        if elapsedTime >= castBar.duration then
            castBar:Hide()
            castBar.casting = false
            return
        end
        local progress = elapsedTime / castBar.duration
        castBar.bar:SetValue(progress)
        castBar.timer:SetText(string.format("%.1f", castBar.duration - elapsedTime))
        if barWidth > 0 then
            castBar.spark:ClearAllPoints()
            castBar.spark:SetPoint("CENTER", castBar.bar, "LEFT", barWidth * progress, 0)
        end
...
local function OnEvent(self, event, ...)
    ...
    elseif event == "UNIT_SPELLCAST_STOP" or event == "UNIT_SPELLCAST_FAILED" or event == "UNIT_SPELLCAST_FAILED_QUIET" then
        local unit = ...
        if unit == "target" then StopCast("target", false) end
    elseif event == "UNIT_SPELLCAST_INTERRUPTED" then
        local unit = ...
        if unit == "target" then StopCast("target", true) end
    ...
    elseif event == "UNIT_SPELLCAST_INTERRUPTIBLE" or event == "UNIT_SPELLCAST_NOT_INTERRUPTIBLE" then
        ...
        if unit == "target" and castBar and castBar:IsShown() and not TomoMod_PreviewMode.IsActive() then
            SetBarColor(event == "UNIT_SPELLCAST_NOT_INTERRUPTIBLE", false)
        end
    elseif event == "PLAYER_TARGET_CHANGED" then
        ...
        if castBar then
            castBar:Hide()
            castBar.casting = false
            if castBar.flashAnim:IsPlaying() then castBar.flashAnim:Stop() castBar.flash:SetAlpha(0) end
        end
        if UnitExists("target") then
            ...
            if name and startTime and endTime then
                ...
                name, text, texture, startTime, endTime, isTradeSkill, notInterruptible = UnitChannelInfo("target")
                if name and startTime and endTime then
                    StartChannel("target", name, texture, startTime, endTime, notInterruptible)
                end
            end
    ...
    elseif event == "UNIT_TARGET" then
        local unit = ...
        if unit == "target" and not TomoMod_PreviewMode.IsActive() then UpdateFlash() end
    end
end
...
function TomoMod_CastBars.UpdateSettings()
    if castBar then UpdateAppearance() end
end
