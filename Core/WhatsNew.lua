...
CHANGELOG = {
    {
        ...
        highlights = {
            ...
            L["wn_345_aura_engine"] or "New — Debuffs and buffs on enemy nameplates were missing for most of a pull, and nothing said so: no error, nothing on the plate, just an empty space where the thing you wanted to see should have been. Everything else in this release makes a refused aura read harmless; none of it makes the aura come back, because the game will not hand it over while a fight is running. So TomoMod stops asking. It now describes what it wants — these auras, this many, laid out like this, styled like this — and the game does the looking and the sorting itself and hands back the finished icons. Nothing on TomoMod's side reads an aura any more, so there is nothing left to refuse. The old way has been removed rather than kept as a spare: this version already requires the patch that provides the new one, and a spare nobody ever runs is a spare that quietly rots. Two bugs were caught on the way out. Hiding a nameplate still reached for the icon list that no longer exists, which would have thrown an error every time a mob died or walked out of range. And an icon created in the middle of a fight came out with no size and drew nothing at all, for good — the game refuses to size it mid-fight just as it refuses to hand over the aura, and the icon was only ever set up once. Icons now remember the size they are waiting for and take it the moment the game allows.",
