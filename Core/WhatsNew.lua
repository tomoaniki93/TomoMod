...
local CHANGELOG = {
    {
        ...
        highlights = {
            ...
            L["wn_335_cdf_aura_probe"] or "CooldownForge: /tm forge log now reports what the game actually let TomoMod read about your buffs — how many buff updates arrived, how many of them carried a readable spell number, the same two figures for combat only, and which spells the game was still willing to name during a fight. It also shows which of the four ways TomoMod has of finding a buff is the one answering, and how often each was even asked, since a method that never works and a method that never gets its turn look exactly the same from outside. When a tracked buff refuses to appear, those lines say whether it is something we can fix or a limit the game is imposing, which nothing else could tell apart. Nothing changes on screen, and the counters start over on each reload — so a clean reading of one fight wants a /reload before the pull.",
            ...
            L["wn_335_cdm_shortcut"] or "Cooldowns: the Cooldowns panel now carries a 'Hide the Cooldown Manager bars' tick next to the Cooldown Studio button. Once your cooldowns are on your own bars, TomoMod's restyling of Blizzard's four viewers has nothing left to do — and the switch for it lived in a different category, which is not where you are when you reach that conclusion. It is the same setting as the one in CD & Resources, worded the other way round. Note that it stops TomoMod dressing those viewers; switching the viewers themselves off is done in Blizzard's Edit Mode.",
