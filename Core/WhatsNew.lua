...
CHANGELOG = {
    {
        ...
        highlights = {
            ...
            L["wn_345_aura_engine"] or "New — Debuffs on enemy nameplates were missing for most of a pull, and nothing said so: no error, nothing on the plate, just an empty space where the thing you wanted to see should have been. Everything else in this release makes a refused aura read harmless; none of it makes the aura come back, because the game simply will not hand it over while a fight is running. So this stops asking. TomoMod now describes what it wants — these debuffs, this many, laid out like this, styled like this — and the game does the looking and the sorting itself and hands back the finished icons. Nothing on TomoMod's side reads an aura any more, so there is nothing left to refuse. This is a first attempt on purpose, and only for the debuffs on nameplates: the old way is still there and still runs on clients that do not have this, so nobody loses their auras to it. If it holds up in real dungeons, everything else that reads auras follows the same route. One thing to watch and worth saying out loud rather than discovering later: an icon that is set up in the middle of a pull may come out with no size and draw nothing, which is the part that needs real fights to confirm.",
