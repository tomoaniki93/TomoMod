...
local function BuildAutomationsTab(parent)
    ...
    -- Prey Tracker (Midnight-only: absent on WoW: Forever)
    if not (TomoMod_Compat and TomoMod_Compat.Blocked("preytracker")) then
        local _, ny = W.CreateSeparator(c, y)
        y = ny
...
local function BuildWaypointTab(parent)
    local scroll = W.CreateScrollPanel(parent)
    local c = scroll.child
    local y = -10

    local function WY_Apply()
        if TomoMod_Waypoint and TomoMod_Waypoint.ApplySettings then
            TomoMod_Waypoint.ApplySettings()
        end
        -- Re-evaluate zone visibility
        if TomoMod_Waypoint and TomoMod_Waypoint.CheckActivePublic then
            TomoMod_Waypoint.CheckActivePublic()
        end
    end

    local _, ny = W.CreateSectionHeader(c, L["section_waypoint"], y)
    y = ny

    -- Zone-only toggle
    local _, ny = W.CreateCheckbox(c, L["opt_way_zone_only"],
        TomoModDB.waypoint and TomoModDB.waypoint.zoneOnly or false, y, function(v)
        if not TomoModDB.waypoint then TomoModDB.waypoint = {} end
        TomoModDB.waypoint.zoneOnly = v
        WY_Apply()
    end)
    y = ny
    ...
    -- Beacon size
    local _, ny = W.CreateSlider(c, L["opt_way_size"],
        TomoModDB.waypoint and TomoModDB.waypoint.beaconSize or 32, 16, 64, 1, y, function(v)
        if not TomoModDB.waypoint then TomoModDB.waypoint = {} end
        TomoModDB.waypoint.beaconSize = v
        WY_Apply()
    end)
    y = ny
    ...
    -- Shape dropdown
    local shapeOptions = {
        { value = "ring",  text = L["way_shape_ring"] },
        { value = "arrow", text = L["way_shape_arrow"] },
    }
    local _, ny = W.CreateDropdown(c, L["opt_way_shape"],
        shapeOptions,
        TomoModDB.waypoint and TomoModDB.waypoint.shape or "ring",
        y, function(v)
        if not TomoModDB.waypoint then TomoModDB.waypoint = {} end
        TomoModDB.waypoint.shape = v
        WY_Apply()
    end)
    y = ny
    ...
    local _, ny = W.CreateColorPicker(c, L["opt_way_color"], TomoModDB.waypoint.color, y, function(r, g, b)
        if not TomoModDB.waypoint then TomoModDB.waypoint = {} end
        if not TomoModDB.waypoint.color then TomoModDB.waypoint.color = {} end
        TomoModDB.waypoint.color.r = r
        TomoModDB.waypoint.color.g = g
        TomoModDB.waypoint.color.b = b
        WY_Apply()
    end)
    y = ny
...
function TomoMod_ConfigPanel_QOL(parent)
    ...
    -- Tabs whose module cannot run on this client are dropped, not
    -- disabled: a tab that opens onto controls for something the game
    -- does not have reads as a bug, not as a limitation.
    if TomoMod_Compat and TomoMod_Compat.IsTabBlocked then
        for i = #tabs, 1, -1 do
            if TomoMod_Compat.IsTabBlocked(tabs[i].key) then
                table.remove(tabs, i)
            end
        end
    end
