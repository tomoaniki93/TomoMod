...
- **Change** — Eleven scanners now ask first: unit frame auras and enemy buffs, nameplate auras and enemy buffs, party dispel and group buff and HoTs, raid debuffs and HoTs and dispel, the shared defensive scan, and CooldownForge's full rescan. Each one used to walk up to forty indices, protecting every single call, to learn what one question answers.
...
#### Nameplates — Guarding The Reads Stopped The Errors And Not The Disappearing Debuffs
...
- **New** — `Modules/Interface/NamePlates/NPAuraContainer.lua`, nameplate debuffs driven by the client's own aura engine. It inverts the flow: TomoMod declares a container, a filter and how to build one button, and the client does the scanning, the sorting and the updating, handing back buttons to style. Nothing in that file ever reads an aura, so nothing in it can be refused.
- **Note** — This is a pilot, deliberately scoped to nameplate debuffs. The scan-and-draw path stays in place and still runs on clients without the template, so nobody loses their auras to a version check. If it holds up, the other ten scanners are the follow-up.
- **Internal** — Availability is probed by building a container and throwing it away, not by asking the API version. The frame type and the template both have to exist, and a version number does not say whether the template is registered.
...
- **Internal** — The filter strings are exact on purpose. The engine batches one scan per distinct filter, so `HARMFUL|PLAYER` and `PLAYER|HARMFUL` describe the same auras and cost two scans.
- **Note** — One known gap, written down in the file rather than papered over: sizing a button is itself refused while auras are restricted, and an unsized button draws nothing. Whether that bites depends on whether the engine runs the initializer once per pooled button or again on reuse, which is a question for a real pull rather than for a reading of the code. There is no retry yet, and the fields a retry would need are present but unread.
