...
- **Fix** — The dungeon table is indexed by challenge map ID, and Blizzard issues a *new* ID when a dungeon returns in a later season. Midnight's Skyreach came back under an ID this table still described as Bloodmaul Slag Mines, so the row was answering for the wrong dungeon: the scoreboard abbreviated it "BSM" and the panel offered the Bloodmaul teleport, a spell the player does not own and a click that does nothing.
