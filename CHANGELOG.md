...
- **Internal** — Three sources in order: the instance registry, then the per-frame scan, then the direct lookup, then a name match. Each exists for a case the one before it does not cover, and the order is cheapest-and-most-reliable first.
...
#### CooldownForge — Measuring What The Client Is Willing To Name Mid-Fight

- **New** — `/tm forge log` now reports the `UNIT_AURA` payload itself: how many auras were handed over, how many arrived with a readable spell ID, the same pair restricted to combat, and the list of IDs the client did name during a fight with a count for each.
- **Internal** — The instance-tracking change above rests on one assumption — that the payload still carries the spell ID at application time while a fight is running, even though the field goes protected afterwards. If the client withholds it there too, the ceiling is Blizzard's and no additional lookup path can move it. Nothing in the addon could tell those two situations apart from the outside: both look like "the buff never shows up".
- **Internal** — The combat list is the interesting half. Whichever IDs come back readable mid-fight are the ones the client is prepared to name, which makes that list a whitelist observed rather than assumed — the difference between a spell TomoMod is failing to track and a spell the client refuses to identify.
- **New** — The same command now also prints which of the four aura sources actually answered. The instance registry, the per-frame scan, the direct lookup and the name match are tried in that order, each one covering what the one before it does not, and until now there was no way to tell a source that never succeeds from a source that never gets asked — the ones ahead of it consume the question first, and both cases look like a line that contributes nothing. Every source is counted twice: how often it was reached, and how often it produced an aura, in combat and overall.
- **New** — The direct lookup gets its own breakdown, because a call that raised an error, a call that returned nothing and a call that returned a usable aura are three different answers that a hit-or-miss tally flattens into one. A protected value, a buff that genuinely is not up and a lookup the client will not perform in combat are told apart here and nowhere else.
- **Note** — The two tallies do not use the same unit and are not meant to add up. A source is only counted when it is reached, so the four rows have different totals by design, and the direct breakdown counts once per candidate ID rather than once per icon — an entry resolving through several linked IDs contributes several lines to it.
- **Note** — Nothing that is drawn changes. The counters live on `CDF.__addStats` and `CDF.__srcStats`, cost one table increment per aura event and per resolution, and start again from zero on a reload — so a measurement is one session's worth, and a clean reading of a single fight wants a `/reload` before the pull.
