...
- **Known limitation** — That visibility pass writes full opacity to every viewer that is *not* hidden, so it can briefly overwrite a configured combat fade until the next alpha event restores it. Binding four viewers now runs the pass once each, which makes the flicker likelier to be seen on login than it was. Hidden viewers are defended by the hook; a faded one is not, and the two deserve the same treatment.
