...
- **New** - All fourteen Bags V4 interface strings ship translated for English, French, German, Spanish, Italian and Brazilian Portuguese, registered by the module itself so no legacy bag locale file is required.
...
- **Changed** - The dashboard's bag toggle now reads and writes `bagsV4.enabled` rather than the legacy flag.
- **Fixed** - Bag slots could appear completely empty. Rendering relied on the private texture and count regions inside Blizzard's `ContainerFrameItemButtonTemplate`, whose names have changed across client versions. TomoMod now draws the icon, the stack count and the cooldown swipe itself, and the Blizzard button is kept purely as the secure input owner with every native decorative region held transparent.
- **Fixed** - Hovering, clicking and dragging a slot could be swallowed by TomoMod's own artwork. The wrapper no longer takes the mouse at all, the secure button sits ten frame levels above it with its hit rectangle reset to the full slot, and every border, count and label TomoMod draws now lives in a mouse-disabled overlay above the cooldown swipe. The middle mouse button is registered on the button itself, so pinning works directly from the grid.
- **New** - Tooltips are raised by the module rather than inherited from whatever container frame Blizzard happens to parent the button to: `SetBagItem` where it exists, the stored item link otherwise. A hover highlight is drawn on the button's own HIGHLIGHT layer, so it cannot be left showing when the pointer leaves.
- **Fixed** - Blizzard's new-item decoration can no longer flash over a TomoMod slot. `NewItemTexture`, `BattlepayItemTexture`, the flash texture and the new-item glow animation are neutralised alongside the normal texture, the icon border, the native icon and the native count.
- **New** - Empty slots are readable again, drawn with the nameplate background and border textures the addon already ships rather than introducing another media set. They are shown only on an empty slot and hidden the moment one is filled.
- **Changed** - Quality borders read more clearly: full opacity above common quality and 0.48 below it, over a darker slot background. The item level moved from the bottom-left to the top-left of the slot and was lightened, so it no longer sits under the stack count.
...
- **New** - The window can no longer be built larger than the screen. Your configured column count stays in your profile, but the live layout is reduced to what the current resolution and scale can actually fit — six to sixteen columns, five to nine visible rows — instead of drawing a frame that runs off the display.
...
- **Internal** - The dashboard, the installer and the presets still write the legacy `bagSkin.enabled` flag during Phase 1. A persisted mirror of that value and a half-second poll detect the external write and route it into Bags V4; both disappear with the dedicated V4 options panel.
