...
- **Change** — The Action Bars options page is a single notice for this release. Every control on it drove `TomoMod_ActionBars` and its layers, which no longer exist, so the page would have been forty-five widgets writing to a schema nothing reads. It says what is happening and offers the move-mode button; lot P5 rebuilds it against the schema the ported module actually uses, and your settings come back with it.
