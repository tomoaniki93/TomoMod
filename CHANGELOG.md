...
#### Group And Raid Frames — The Last Scanners Move Over, And One Deliberately Does Not
...
- **Note** — The dispel *highlight* — the coloured border on the unit frame itself, not on an aura icon — stays on the guarded scan, and that is a decision rather than an omission. The engine colours its own aura buttons; it has no way to answer "what dispel type is on this unit" without us reading the aura. So that one indicator still depends on auras being readable.
