...
- **Fixed** - Player names, channel labels, item links and other Blizzard hyperlinks are clickable again in the Chat V4 message area. The owned message surface explicitly enables hyperlinks and forwards them to Blizzard's native link handler, while TomoMod URL links continue to open the focused copy popup.
- **Fixed** - Chat V4 now has a real mover in TomoMod Layout Mode. Its full-panel overlay moves the underlying Blizzard `ChatFrame1` and native input box in real time, records the new position through both the shared TomoMod layout engine and Blizzard's chat persistence, and releases the message surface's mouse capture during TomoMod Layout Mode or Blizzard Edit Mode before restoring hyperlinks and scrolling.
...
- **New** - Spells, items and equipped trinket slots can be added directly, with quick presets for both trinkets and every registered item preset. Blizzard-tracked bars can be resynchronised from their current Essentials, Utilities or Tracked Buffs list while preserving manual entries and every setting on entries that remain.
...
- **New** - Chat V4 and Bags V4 now declare their real movable anchors in the default database and module manifest. Layout migration, resolution-reference stamping, captured resolution presets and selective imports can include the current V4 frames instead of the obsolete legacy chat anchors; the new frame and module labels are translated in all six supported languages.
- **Fixed** - Bags V4 now saves the window's physical edges after a drag through `Layout.Save` and restores them through `Layout.Apply`. It no longer reuses the anchor offsets from before the drag or multiplies its position by the frame scale, preventing the bag window from stepping away from the chosen location after later moves or reloads.
- **Fixed** - Layout Mode recognises the actual `TUI_ActionBar_*` and `TomoMod_BagsV4_*` frame families, so their cog buttons open Action Bars and Skins respectively. The frame-name collector now understands non-TomoMod and dynamically assembled names instead of silently treating those routes as unused.
- **New** - Layout V4.1 adds optional Pixel Perfect persistence. Positions saved through the shared layout engine can now snap to the nearest physical display pixel, using the real screen resolution to derive pixel size while leaving existing Layout V4 behaviour unchanged when the option is disabled.
- **New** - Layout Mode gains a Pixel Perfect / Nudger panel for the movable element under the cursor. It can move supported frames by 1, 5 or 10 physical pixels, show their centre coordinates, centre either axis or restore the default position, while protected frames remain untouched in combat.
- **New** - Player ↔ Target mirroring keeps both unit frames horizontally symmetric around the physical centre of the screen and at the same height. Enabling it immediately aligns Target from Player, and later saves to either frame update the other without recursive writes.
- **Changed** - Player, Target and Focus frame positions now save and restore through the shared Layout V4 engine instead of a separate absolute-coordinate path. They gain resolution-aware scaling, Pixel Perfect snapping and linked-anchor notifications while retaining compatibility fallbacks for early load-order cases.
- **Fixed** - The Pixel Perfect / Nudger panel no longer uses Unicode direction triangles, the bidirectional arrow or long coordinate dashes that some WoW fonts render as missing-glyph diamonds. Its directional controls, Player/Target label and unavailable coordinates now use font-safe plain-text markers.
- **Changed** - The precision-placement panel is now named TomoLayout and stays hidden until the player clicks or begins dragging a supported movable element. Selection hooks prefer each module's dedicated mover overlay where available, avoiding changes to the underlying protected unit button.
...
- **Changed** - Layout mover overlays share one reusable style: an opaque azure-to-near-white gradient, a high-contrast border and centred azure `#2E9DD8` text in Poppins Medium with WoW's black outline and no white shadow. The inner name band has been removed so small movers no longer look like a box inside a box, while repeated styling calls still reuse existing textures.
- **Changed** - Custom labels that do not use the complete shared overlay now use the same typography helper, including Party, Raid and Arena Frames, Auras, Chat V4, action and extra-action bars, SkyRide, Micro Menu, Prey Tracker, Battle Resurrection, Tooltip and Cooldown Manager hints.
...
- **Internal** - The out-of-game brand identity bench scans more than 100 Lua sources, verifies hexadecimal/RGB consistency and the palette hierarchy, rejects the previous mint and unexpected residual teal accents, checks ten migrated mover overlays plus thirteen custom labels, and fails if a module recreates its own label style.
