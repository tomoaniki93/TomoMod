...
TomoMod_RegisterLocale("enUS", {
    ...
    ["wn_344_dm_config"] = "New — A Damage Meter page in TomoMod's options. For now it is a doorway rather than the settings themselves: it tells you where the meter's own settings window is, opens it, and shows or hides the meter windows. That is on purpose — a half-copied set of options that disagreed with the real ones would be worse than a page that is honest about being a shortcut, and the real integration is the next step. In the two cases where there is nothing to open — the standalone addon has taken over, or your client has no damage meter data — the page says which, rather than offering a button that does nothing.",
