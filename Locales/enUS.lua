...
TomoMod_RegisterLocale("enUS", {
    ...
    ["time_tooltip_left_click"]         = "|cff2e9dd8Left-click:|r Calendar",
    ["time_tooltip_right_click"]        = "|cff2e9dd8Right-click:|r Server / Local",
    ["time_tooltip_shift_right"]        = "|cff2e9dd8Shift + Right-click:|r 12h / 24h",
    ...
    ["opt_chat_skin_bg_alpha"]           = "Frame background opacity",
    ["opt_chat_message_bg_alpha"]        = "Message area opacity",
    ...
    ["tmt_unlock_msg"]              = "|cff2e9dd8TomoMod|r M+ Tracker: Frame unlocked \226\128\148 drag to reposition.",
    ["tmt_lock_msg"]                = "|cff2e9dd8TomoMod|r M+ Tracker: Frame locked.",
    ["tmt_reset_msg"]               = "|cff2e9dd8TomoMod|r M+ Tracker: Position reset.",
    ["tmt_unknown_cmd"]             = "|cff2e9dd8TomoMod|r M+ Tracker: Unknown command.",
    ["tmt_key_level"]               = "+%d",
    ...
    ["tmt_splits_cleared"]            = "|cff2e9dd8TomoMod|r M+ Tracker: recorded times cleared.",
    ...
    ["tmt_preview_active"]          = "|cff2e9dd8TomoMod|r M+ Tracker: Preview active \226\128\148 |cFF55B400/tmt lock|r to lock.",
    ...
    ["ts_reset_msg"]                = "|cff2e9dd8TomoMod|r Scoreboard: Position reset.",
    ["ts_no_data"]                  = "|cff2e9dd8TomoMod|r Scoreboard: No dungeon data available.",
    ...
    ["mhub_tp_not_learned"]          = "|cff2e9dd8TomoMod|r: Teleport spell not learned.",
    ...
    -- INSTALLER
    -- =====================
    ["ins_header_title"]             = "|cff2e9dd8Tomo|r|cffe4e4e4Mod|r  —  Setup Wizard",
    ...
    ["ins_welcome_desc"]             = "This wizard will guide you through |cff2e9dd816 steps|r to configure TomoMod to\nyour preferences: unit frames, party frames, castbars, nameplates, action bars,\nresources, skins, sound, Mythic+, QOL, optimizations and SkyRide.\n\nAll these options can be changed anytime via |cff2e9dd8/tm|r.",
    ...
    ["ins_done_recap"]               = "Your TomoMod configuration is saved. Here are some reminders:\n\n|cff2e9dd8/tm|r              |TInterface\\BUTTONS\\UI-SpellbookIcon-NextPage:0|t  Open the configuration panel\n|cff2e9dd8/tm sr|r           |TInterface\\BUTTONS\\UI-SpellbookIcon-NextPage:0|t  Unlock and move elements\n|cff2e9dd8/tm install|r      |TInterface\\BUTTONS\\UI-SpellbookIcon-NextPage:0|t  Relaunch this installer\n\nAll options configured here can be changed anytime\nfrom the corresponding panels in the TomoMod GUI.\n\nA |cff2e9dd8UI reload|r is required to apply certain changes\n(nameplates, skins, UnitFrames).",
    ...
    ["bags_v4_quality_style"]            = "Item quality",
    ["bags_v4_quality_tint"]             = "Tinted slot",
    ["bags_v4_quality_border"]           = "Coloured frame",
    ["bags_v4_quality_both"]             = "Both",
    ["bags_v4_quality_none"]             = "None",
    ...
    ["wn_btn_ok"]                        = "Got it!",
    ["wn_footer"]                        = "All settings can be changed anytime via |cff2e9dd8/tm|r.",
    ...
    -- 3.1.2
    ["wn_312_brand"]         = "Brand color updated from #0cd29f to #2e9dd8 (mint green) across the entire UI — title bar, panels, chat messages, popups and default color values.",
    ...
    ["wn_400_context_tools"] = "New — A profile swap no longer closes the tools you had open. The diagnostics panel, the addon detector and the studios stay exactly as you left them across a content change: opening a studio in a raid and then stepping into a key does not shut it. Loading a profile yourself still replaces everything, because that is what you are asking for by clicking.",
    ["wn_400_context_combat"] = "New — A content change that lands mid-pull waits. Slotting a key puts you in combat straight away, so the swap is held until the fight is over and applied then, rather than rebuilding your frames on the pull. And the two profiles are compared before anything is applied: you are only asked to reload when the swap really does change which modules are switched on — a different size, colour or position is applied on the spot, without a word.",
    ["wn_400_ready_tracker"] = "New — A readiness tracker for group content. TomoMod now watches your flask, your food buff and the oil on your weapon together, and tells you at a glance whether all three are up. It sits next to the clock under the minimap and can be opened anywhere, but it only lights up where it matters: the glyph stays white while you are solo, and turns green or red as soon as you set foot in a dungeon, a raid, a scenario or a delve.",
    ["wn_400_ready_tracker_button"] = "New — The tracker sits as a small button next to the clock under the minimap: teal when everything is up, red as soon as something is missing. Hover it to read the three lines one by one, left click to open a panel showing each buff's icon and remaining time, and right click to move the button to the other side of the clock. Carrying a weapon in each hand asks for two oils and shows you how many are on; a shield or a held off-hand item is not counted, since neither of them can be oiled. The same two settings are in Options, under General, in the information panel section.",
    ["wn_400_live_toggles"] = "New — Most of TomoMod's modules can now be switched on and off without reloading the interface. 27 of the 62 modules take effect the moment you flip the switch, instead of setting a flag and asking you to reload before anything happens. The rest still need a reload, and now they only ask when there is a real reason for it.",
    ["wn_400_combat_defer"] = "New — A switch flipped in the middle of a fight is no longer refused, nor applied halfway. A change that would have to rebuild a protected frame is held until you leave combat and applied automatically then, and TomoMod tells you it is waiting. The changes that are safe in combat — fast loot, auto-repair, the alerts and the trackers — go through immediately, because making you leave a fight to turn off automatic looting would be theatre.",
    ["wn_400_dependencies"] = "New — Modules know what they depend on. Turning one off now also turns off the ones that cannot work without it, and names them; turning one on warns you when something it needs is still off. Nothing is left half-connected in silence any more.",
    ["wn_400_reload_honest"] = "Changed — The reload prompt is no longer TomoMod's answer to every single toggle. It appears for the 14 modules that genuinely cannot undo their work while the game is running — secure buttons that may not be respawned in combat, and hooks that cannot be removed once installed — and nowhere else.",
    ["wn_400_reload_batch"] = "New — When a change really does need a reload, TomoMod asks once instead of once per click. Ticking five boxes in a row raises a single question at the end, and choosing Later keeps your answer rather than dropping it: a small banner at the top of the screen then lists what is waiting, and reloads when you are ready. Undoing a change before you reload removes it from that list on its own — putting a setting back the way it was when you logged in is no longer something you have to reload to confirm.",
    ["wn_400_inventory"] = "New — /tm modules prints everything TomoMod contains, grouped the way the options are, with each module's state and whether it can be changed live or needs a reload. /tm modules <key> flips one straight from the chat box. Useful on its own, and the fastest way to find out which of your modules are live and which are not.",
    ["wn_400_foundation"] = "Note — Underneath all of the above, TomoMod now keeps one single description of its own contents: 68 entries covering every setting it saves, in nine groups, with 29 movable elements declared. Nothing changes on screen because of it. What it buys is already showing: the content profiles above read from that one list, and importing only part of a profile and the new layout engine will read from it too, instead of each rebuilding its own slightly different version.",
    ...
    ["wn_400_units_workspace"] = "New — Units now has the same focused workspace navigation. UnitFrames, Nameplates, Party Frames and Raid Frames are listed directly beneath Units in the sidebar, so the pages you use together stay together. Their existing controls, previews and nested tabs are unchanged.",
    ...
    -- 3.6.5 — What's New
    -- =====================
    ["wn_365_mplus_stats_compare"] = "Fix — The Mythic+ Studio's Statistics page no longer throws an error once you have two or more recorded runs. The run comparison card formats each run's time, and the formatter it called was not yet in scope where that card is built, so the page stopped drawing halfway through and the comparison never appeared. A fresh profile looked fine, because the card is only built from the second recorded key onward.",
    ["wn_365_mplus_compare_columns"] = "Fix — The run comparison table on the Mythic+ Studio's Statistics page draws inside its card again. Its three value columns — run A, run B and the difference — were positioned past the right-hand edge of the panel, so the row labels appeared with no figures next to them. The columns, and the two run names above them, are now placed from the card's top-left corner and stay within it.",
    ["wn_365_mplus_compare_polish"] = "New — The comparison table reads as a table: a header band, a rule in front of each column, one row in two shaded, and the figures centred under their heading. The difference column is coloured now — green when the second run is the better one, red when it is worse, grey when there is nothing to compare — using the right direction for each line: a higher key level is an improvement, and so is a lower time, death count, enemy-forces time or boss split.",
    ["wn_365_teleport_launcher"] = "Changed — The Mythic+ teleport button on your character sheet is now a small, discreet icon placed just to the left of the sheet's close button, instead of the large icon that sat in the top-right corner. It opens exactly the same teleport panel, and behaves the same way — it simply no longer competes with the character sheet's own header for your attention.",
    ["wn_365_chat_secret_channel"] = "Fix — Channel notices no longer break the chat window. The game now hands the chat handler a protected value instead of a plain name for some channel notices, and comparing such a value with text is not allowed — so working out which channel a notice belonged to could fail before the notice was ever printed. TomoMod's chat skin now checks whether the value is protected before comparing it, and join, leave, invite and wrong-password notices are handled normally again.",
