...
TomoMod_RegisterLocale("deDE", {
    ...
    ["wn_335_cdf_aura"] = "CooldownForge: Ein Symbol kann einen Buff statt einer Abklingzeit verfolgen. Stellen Sie einen Eintrag auf „verfolgter Buff“, und er bleibt vom Bildschirm fern, solange der Buff fehlt, und erscheint dann mit Restdauer und Stapeln, solange er aktiv ist — ein Proc kann so auf derselben Leiste neben der Fähigkeit sitzen, die ihn auslöst. Sie können auch eine abweichende Buff-ID angeben, für Procs, die von einem Zauber gewährt und als ein anderer angewendet werden. In Mythisch+ lässt das Spiel Addons die Dauer eines Buffs womöglich nicht auslesen; das Symbol erscheint und verschwindet dort trotzdem korrekt, nur eben ohne Zeitanzeige.",
